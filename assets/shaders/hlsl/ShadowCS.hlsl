...
Shadow_Shadow_RootConstants(b1)
...
float4x4 GetViewProjMatrix(in Light light, in float2 uv, in uint index) {
...
void CS(uint2 DTid : SV_DispatchThreadID) {
	...
	if (needCube) {
		...
		const float shadowFactor = all(viewProj != (float4x4)0.f) ? CalcShadowFactorCubeCS(gi_ZDepthTexArray, gsamShadow, viewProj, posW.xyz, uv, index) : 1.f;
