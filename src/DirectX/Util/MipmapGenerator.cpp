...
BOOL MipmapGeneratorClass::Initialize(Locker<ID3D12Device5>* const device, ShaderManager* const manager) {
	md3dDevice = device;
...
BOOL MipmapGeneratorClass::BuildPSO() {
	D3D12Util::Descriptor::PipelineState::Builder builder;
	{
		...
		
		{
			...
			builder.Enqueue(psoDesc, IID_PPV_ARGS(&mPSOs[PipelineState::EG_GenerateMipmap]));
		}
		{
			{
			...
			builder.Enqueue(psoDesc, IID_PPV_ARGS(&mPSOs[PipelineState::EG_JustCopy]));
		}
	}
