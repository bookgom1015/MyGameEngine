...
BOOL EquirectangularConverterClass::Initialize(Locker<ID3D12Device5>* const device, ShaderManager* const manager) {
	md3dDevice = device;
...
BOOL EquirectangularConverterClass::BuildRootSignature(const StaticSamplers& samplers) {
	...
	// ConvEquirectToCube
	{
		...
		builder.Enqueue(rootSigDesc, IID_PPV_ARGS(&mRootSignatures[RootSignature::E_ConvEquirectToCube]));
	}
	// ConvCubeToEquirect
	{
		CD3DX12_DESCRIPTOR_RANGE texTables[1] = {}; UINT index = 0;
		...
		builder.Enqueue(rootSigDesc, IID_PPV_ARGS(&mRootSignatures[RootSignature::E_ConvCubeToEquirect]));
	}
...
BOOL EquirectangularConverterClass::BuildPSO() {
	...
	// ConvEquirectToCube
	{
		...
		builder.Enqueue(psoDesc, IID_PPV_ARGS(&mPSOs[PipelineState::EG_ConvEquirectToCube]));
	}
	// ConvCubeToEquirect
	{
		...
		builder.Enqueue(psoDesc, IID_PPV_ARGS(&mPSOs[PipelineState::EG_ConvCubeToEquirect]));
	}

	{
		...
		CheckHRESULT(builder.Build(device));
	}
