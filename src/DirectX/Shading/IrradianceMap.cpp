...
BOOL IrradianceMapClass::Initialize(Locker<ID3D12Device5>* const device, ShaderManager* const manager) {
	md3dDevice = device;
...
BOOL IrradianceMapClass::BuildDescriptors() {
	D3D12Util::Descriptor::View::Builder builder;
	{
...

		{
			...
			builder.Enqueue(mDiffuseIrradianceCubeMap->Resource(), srvDesc, mhDiffuseIrradianceCubeMapCpuSrv);
		}
		{
			srvDesc.TextureCube.MipLevels = MaxMipLevel;

			builder.Enqueue(mEnvironmentCubeMap->Resource(), srvDesc, mhEnvironmentCubeMapCpuSrv);
			builder.Enqueue(mPrefilteredEnvironmentCubeMap->Resource(), srvDesc, mhPrefilteredEnvironmentCubeMapCpuSrv);
		}
	}
	{
		D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
		...
		{
			builder.Enqueue(mDiffuseIrradianceEquirectMap->Resource(), srvDesc, mhDiffuseIrradianceEquirectMapCpuSrv);
		}
		{
			for (UINT i = 0; i < MaxMipLevel; ++i)
				builder.Enqueue(mPrefilteredEnvironmentEquirectMaps[i]->Resource(), srvDesc, mhPrefilteredEnvironmentEquirectMapCpuSrvs[i]);
		}
		{
			srvDesc.Format = IntegratedBrdfMapFormat;

			builder.Enqueue(mIntegratedBrdfMap->Resource(), srvDesc, mhIntegratedBrdfMapCpuSrv);
		}
	}
	{
...

		{
...

			{
				...
				builder.Enqueue(mDiffuseIrradianceCubeMap->Resource(), rtvDesc, mhDiffuseIrradianceCubeMapCpuRtv);
			}
			for (UINT mipLevel = 0; mipLevel < MaxMipLevel; ++mipLevel) {
				rtvDesc.Texture2DArray.MipSlice = mipLevel;

				builder.Enqueue(mEnvironmentCubeMap->Resource(), rtvDesc, mhEnvironmentCubeMapCpuRtvs[mipLevel]);
			}
			for (UINT mipLevel = 0; mipLevel < MaxMipLevel; ++mipLevel) {
				rtvDesc.Texture2DArray.MipSlice = mipLevel;

				builder.Enqueue(mPrefilteredEnvironmentCubeMap->Resource(), rtvDesc, mhPrefilteredEnvironmentCubeMapCpuRtvs[mipLevel]);
			}
		}
	}
	{
		...
		for (UINT i = 0; i < MaxMipLevel; ++i) {
			...
			builder.Enqueue(mEquirectangularMap->Resource(), rtvDesc, mhEquirectangularMapCpuRtvs[i]);
		}
		{
			rtvDesc.Texture2D.MipSlice = 0;

			builder.Enqueue(mDiffuseIrradianceEquirectMap->Resource(), rtvDesc, mhDiffuseIrradianceEquirectMapCpuRtv);
		}
		for (UINT i = 0; i < MaxMipLevel; ++i) {
			builder.Enqueue(mPrefilteredEnvironmentEquirectMaps[i]->Resource(), rtvDesc, mhPrefilteredEnvironmentEquirectMapCpuRtvs[i]);
		}
		{
			rtvDesc.Format = IntegratedBrdfMapFormat;
			rtvDesc.Texture2D.MipSlice = 0;

			builder.Enqueue(mIntegratedBrdfMap->Resource(), rtvDesc, mhIntegratedBrdfMapCpuRtv);
		}
	}
...
BOOL IrradianceMapClass::Update(
...

	{
		...
		device->CreateShaderResourceView(mTemporaryEquirectangularMap->Resource(), &srvDesc, mhTemporaryEquirectangularMapCpuSrv);
	}
...

	{
	...
	
		{
			...
			srvDesc.Texture2D.PlaneSlice = 0;
			srvDesc.Format = HDR_FORMAT;
			device->CreateShaderResourceView(mEquirectangularMap->Resource(), &srvDesc, mhEquirectangularMapCpuSrv);
		}
...
BOOL IrradianceMapClass::BuildResources() {
	...
	}
	{
		...
		WCHAR name[] = L"Irradiance_PrefilteredEnvironmentEquirectangularMap_";
		for (UINT i = 0; i < MaxMipLevel; ++i) {
			...
			builder.Enqueue(
				mPrefilteredEnvironmentEquirectMaps[i].get(),
				CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),
				D3D12_HEAP_FLAG_NONE,
				texDesc,
				D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE,
...
BOOL IrradianceMapClass::Load(
	...
	device->CreateShaderResourceView(dst->Resource(), &srvDesc, hDesc);
