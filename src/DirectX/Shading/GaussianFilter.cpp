...
BOOL GaussianFilterClass::Initialize(Locker<ID3D12Device5>* const device, ShaderManager* const manager) {
	md3dDevice = device;
...
BOOL GaussianFilterClass::BuildPSO() {
	D3D12Util::Descriptor::PipelineState::Builder builder;
	{
		...
		for (UINT i = 0; i < FilterType::Count; ++i) {
			...
			builder.Enqueue(psoDesc, IID_PPV_ARGS(&mPSOs[static_cast<FilterType>(i)]));
		}
	}
