...
BOOL ToneMappingClass::Initialize(Locker<ID3D12Device5>* const device, ShaderManager* const manager, UINT width, UINT height) {
	md3dDevice = device;
...
BOOL ToneMappingClass::BuildPSO() {
	...
	// ToneMapping
	{
		...
		builder.Enqueue(psoDesc, IID_PPV_ARGS(&mPSOs[PipelineState::E_ToneMapping]));
	}
	// JustResolving
	{
		{
		...
		builder.Enqueue(psoDesc, IID_PPV_ARGS(&mPSOs[PipelineState::E_JustResolving]));
	}
...
BOOL ToneMappingClass::BuildDescriptors() {
...

	{
		...
		device->CreateShaderResourceView(mIntermediateMap->Resource(), &srvDesc, mhIntermediateMapCpuSrv);
		device->CreateRenderTargetView(mIntermediateMap->Resource(), &rtvDesc, mhIntermediateMapCpuRtv);
	}
