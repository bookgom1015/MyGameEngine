...
BOOL SSAOClass::PrepareUpdate(ID3D12GraphicsCommandList* const cmdList) {
	...
	CheckReturn(BuildRandomVectorTexture(device, cmdList));
...
BOOL SSAOClass::BuildDescriptors() {
	...
	device->CreateShaderResourceView(mRandomVectorMap->Resource(), &srvDesc, mhRandomVectorMapCpuSrv);

	srvDesc.Format = AOCoefficientMapFormat;
	for (UINT i = 0; i < 2; ++i) {
		device->CreateShaderResourceView(mAOCoefficientMaps[i]->Resource(), &srvDesc, mhAOCoefficientMapCpuSrvs[i]);
	}
	...
	for (UINT i = 0; i < 2; ++i) {
		device->CreateRenderTargetView(mAOCoefficientMaps[i]->Resource(), &rtvDesc, mhAOCoefficientMapCpuRtvs[i]);
	}
...
BOOL SSAOClass::BuildResources(UINT width, UINT height) {
...

	{
		...
		for (UINT i = 0; i < 2; ++i) {
			...
			CheckReturn(mAOCoefficientMaps[i]->Initialize(
				device,
...
BOOL SSAOClass::BuildRandomVectorTexture(ID3D12Device5* const device, ID3D12GraphicsCommandList* const cmdList) {
	//
	...
	mRandomVectorMapUploadBuffer.release();
